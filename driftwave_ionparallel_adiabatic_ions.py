...
def fun(t, solution, T, n, Ln, Lt, B, m, q):
    ...
    # ve is divided by cs to have velocity in 1/cs space (~O(1)).
    dndt = -n*(ve/(cs*Ln) + duz)
    ...
    #dudt = dudt_abs
    # Ion energy balance equation. Use also here (1/cs) for the velocities.
    #dpdt = -(5/3)*n*T*duz - (ve/cs)*n*T*(1/Ln + 1/Lt)
...
if __name__=='__main__':
    ...
    # These are varied.
    Ln = 10          # Density gradient scale length in units of m.
    Lt = 0.001       # Temperature gradient scale length in units of m. mem=0.4
    #ky = 0.0        # ky for the perturbation
    #kz = 1.0        # kz for the perturbation
 
    use_solve_ivp = False # A flag to choose between scipy.integrate.solve_ivp
    ...
    if use_solve_ivp:
        ...
        solution = forward_dt(S0.ravel(), T, n, Ln, Lt, B, m, q, dt=1e-4)
        nt = 10000
        phivec = []
        for i in range(nt):
